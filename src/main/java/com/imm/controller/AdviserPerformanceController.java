...
@Controller
@RequestMapping("AdviserMng/adviserPer")
public class AdviserPerformanceController extends BaseController {

	final static Log log = LogFactory.getLogger(AdviserPerformanceController.class);
	@Autowired
	...
	public String listByPage(HttpServletRequest request, HttpServletResponse response,
		...
		return "AdviserMng/adviserper/index";
	}
	...
	private String returnEdit(ModelMap model, AdviserVo advo) {
		...
		return "AdviserMng/adviserper/edit";
	}
...
/*
		...
		if (null != prdId) {
			productVo = productService.getProductDetailById(prdId);
			*//** 该产品不是信托，加载信托公司的银行卡信息 *//*
			if (productVo.getPrdType() != ProductTypeEnum.p2p.getCode()
					|| productVo.getOperationType() == ProductConstant.OPERATION_TYPE_TRUE) {
				ProductPayAccountVo productPayAccountVo = productService.getProductPayAccountByPrdId(prdId);
				model.addAttribute("productPayAccountVo", productPayAccountVo);
			}
			getPrrLists(model, prdId);
			*//** 将产品记录中的供应商id查询出name，保存并在view显示 *//*
			String provName = providerService.getProviderById(productVo.getPrvId()).getName();
